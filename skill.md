...
Run:
```bash
npx @fullstacked-labs/pocketdev <port>
```
Or if the port is unknown or standard (3000, 5173):
```bash
npx @fullstacked-labs/pocketdev
```
## What it does
...
- When the user asks to test or preview their app on a phone: invoke `npx @fullstacked-labs/pocketdev <port>`.
