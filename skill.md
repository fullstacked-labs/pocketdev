...
npx devhop [port]
```
Or if the port is unknown or standard (3000, 5173, etc.):
```bash
npx devhop
```

## What it does
1. Creates an ephemeral, trusted HTTPS tunnel via Cloudflare Quick Tunnels.
...
- When the user asks to test or preview their app on a phone: invoke `npx devhop <port>`.
